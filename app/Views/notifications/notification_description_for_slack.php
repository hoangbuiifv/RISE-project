...
//show data from hook
try {
    $notification_config = app_hooks()->apply_filters('app_filter_notification_description', $notification);
    if ($notification_config && get_array_value($notification_config, "notification_description_for_slack")) {
        echo get_array_value($notification_config, "notification_description_for_slack");
    }
} catch (\Exception $ex) {
